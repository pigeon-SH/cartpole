...
def evaluate(env_name, scenario_n, obs_select, frame_num, load_best):
    ...
    if load_best:
        policy_net = torch.load('best_policy_net_' + obs_select + '.pt')
        print("-----Best Network Evaluation-----")
    else:
        policy_net = torch.load('policy_net_' + obs_select + '.pt')
        print("-----Final Network Evaluation-----")
    policy_net.eval()

    for episode in range(scenario_n):
        video_path = 'evaluate/video_' + obs_select + '_' + str(episode) + '.mp4'
...
if __name__ == "__main__":
    ...
    if load_best == None:
        evaluate("CartPole-v1", episode_num, obs_select, frame_num, True)
        evaluate("CartPole-v1", episode_num, obs_select, frame_num, False)
    elif load_best:
        evaluate("CartPole-v1", episode_num, obs_select, frame_num, True)
    else:
        evaluate("CartPole-v1", episode_num, obs_select, frame_num, False)